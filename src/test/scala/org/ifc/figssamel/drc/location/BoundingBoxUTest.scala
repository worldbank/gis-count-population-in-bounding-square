...
class BoundingBoxUTest extends FlatSpecLike with Matchers {
  ...
  "form a square with an edge length of the specified edge length" in {
    ...
    val actual250m = BoundingBox.from(centerPoint, 0.250)
    val actual500m = BoundingBox.from(centerPoint, 0.500)

    actual250m shouldEqual expectedBox250m
    actual500m shouldEqual expectedBox500m
  }
